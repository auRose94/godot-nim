...
proc `eventIndex=`*(self: InputEventAction; index: int32) =
  var methodbind {.global.}: MethodBindPtr
  if unlikely(methodbind.isNil):
    let name = api.newStringName "set_event_index"
    methodbind = interface_ClassDB_getMethodBind(addr className InputEventAction, addr name, 1286410249)
  var `?param` = [getPtr index]
  interface_Object_methodBindPtrCall(methodbind, getOwner self, addr `?param`[0], nil)
proc eventIndex*(self: InputEventAction): int32 =
  var methodbind {.global.}: MethodBindPtr
  if unlikely(methodbind.isNil):
    let name = api.newStringName "get_event_index"
    methodbind = interface_ClassDB_getMethodBind(addr className InputEventAction, addr name, 3905245786)
