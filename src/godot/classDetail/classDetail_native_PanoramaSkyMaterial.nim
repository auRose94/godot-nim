...
proc `energyMultiplier=`*(self: PanoramaSkyMaterial; multiplier: Float) =
  var methodbind {.global.}: MethodBindPtr
  if unlikely(methodbind.isNil):
    let name = api.newStringName "set_energy_multiplier"
    methodbind = interface_ClassDB_getMethodBind(addr className PanoramaSkyMaterial, addr name, 373806689)
  var `?param` = [getPtr multiplier]
  interface_Object_methodBindPtrCall(methodbind, getOwner self, addr `?param`[0], nil)
proc energyMultiplier*(self: PanoramaSkyMaterial): Float =
  var methodbind {.global.}: MethodBindPtr
  if unlikely(methodbind.isNil):
    let name = api.newStringName "get_energy_multiplier"
    methodbind = interface_ClassDB_getMethodBind(addr className PanoramaSkyMaterial, addr name, 1740695150)
