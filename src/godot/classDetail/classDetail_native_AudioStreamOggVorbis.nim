...
proc loadFromBuffer*(_: typedesc[AudioStreamOggVorbis]; buffer: PackedByteArray): GD_ref[AudioStreamOggVorbis] =
...
proc loadFromFile*(_: typedesc[AudioStreamOggVorbis]; path: String): GD_ref[AudioStreamOggVorbis] =
  var methodbind {.global.}: MethodBindPtr
  if unlikely(methodbind.isNil):
