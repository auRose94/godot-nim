...
proc originalName*(self: GLTFMesh): String =
  var methodbind {.global.}: MethodBindPtr
  if unlikely(methodbind.isNil):
    let name = api.newStringName "get_original_name"
    methodbind = interface_ClassDB_getMethodBind(addr className GLTFMesh, addr name, 2841200299)
  var ret: encoded String
  interface_Object_methodBindPtrCall(methodbind, getOwner self, nil, addr ret)
  (addr ret).decode_result(String)
proc `originalName=`*(self: GLTFMesh; originalName: String) =
  var methodbind {.global.}: MethodBindPtr
  if unlikely(methodbind.isNil):
    let name = api.newStringName "set_original_name"
    methodbind = interface_ClassDB_getMethodBind(addr className GLTFMesh, addr name, 83702148)
...
proc getAdditionalData*(self: GLTFMesh; extensionName: StringName): Variant =
  ...
  if unlikely(methodbind.isNil):
    ...
    methodbind = interface_ClassDB_getMethodBind(addr className GLTFMesh, addr name, 2138907829)
  var `?param` = [getPtr extensionName]
  var ret: encoded Variant
  interface_Object_methodBindPtrCall(methodbind, getOwner self, addr `?param`[0], addr ret)
  (addr ret).decode_result(Variant)
proc setAdditionalData*(self: GLTFMesh; extensionName: StringName; additionalData: Variant) =
  var methodbind {.global.}: MethodBindPtr
  if unlikely(methodbind.isNil):
    let name = api.newStringName "set_additional_data"
    methodbind = interface_ClassDB_getMethodBind(addr className GLTFMesh, addr name, 3776071444)
